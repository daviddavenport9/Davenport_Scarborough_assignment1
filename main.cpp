...
int main(const int argc, const char * array[]){
  if (argc != 3) { // if incorrect parameters
    cout << "Error: There are not 2 arguments being passed in" << endl;
    exit(0);
  }if ((strcmp(array[1], "instructors.txt") != 0) && (strcmp(array[2], "students.txt") == 0)){ // instructors.txt not found
    cout << "Error: cannot parse instructors information from file " << array[1] << endl;
    exit(0);
  }if ((strcmp(array[1], "instructors.txt") == 0) && (strcmp(array[2], "students.txt") != 0)){ // students.txt not found
    cout << "Error: cannot parse students information from file " << array[2] << endl;
    exit(0);
  }if ((strcmp(array[1], "instructors.txt") != 0) && (strcmp(array[2], "students.txt") != 0)){ // either files not found
    ...
    Instructor instructor;
      ...
      while (queryValue){
        ...
        if(queryOption == 1){
          ...
          cin >> gradeType;
          if((gradeType < 1) || (gradeType > 5)){ // check for invalid input
            cout << "Invalid Option. Please enter a valid option." << endl;
            continue;
          } // if                                                                                                                                                
          cout << "" << endl;//formatting
